...
export default function FlashcardStatsScreen() {
  ...
  const prevMonth = () => { setCalendarDate(new Date(calYear, calMonth - 1, 1)); setSelectedDay(null); };
  const nextMonth = () => {
    const now = new Date();
    const next = new Date(calYear, calMonth + 1, 1);
    if (next <= new Date(now.getFullYear(), now.getMonth() + 1, 1)) {
      setCalendarDate(next); setSelectedDay(null);
    }
  };
      ...
      <View style={[styles.statsCard, { backgroundColor: colors.card }]}>
        <View style={styles.statsRow}>
          <View style={styles.statItem}>
            <Text style={[styles.statValue, { color: colors.primary }]}>{totalReviewed}</Text>
      ...
      {todayData && (
        <>
          ...
          <View style={[styles.statsCard, { backgroundColor: colors.card }]}>
            <View style={styles.statsRow}>
              <View style={styles.statItem}>
                <Text style={[styles.statValue, { color: colors.primary }]}>{todayData.reviewed}</Text>
                <Text style={[styles.statLabel, { color: colors.textMuted }]}>Cards</Text>
              </View>
              <View style={styles.statItem}>
                <Text style={[styles.statValue, { color: colors.primary }]}>
      ...
      {totalReviewed === 0 && (
        <View style={styles.emptyContainer}>
          ...
          <Text style={[styles.emptySubtitle, { color: colors.textMuted }]}>
            Start a flashcard session to track your progress
          </Text>
        </View>
      )}
    </ScrollView>
...
const styles = StyleSheet.create({
  ...
  sectionTitle: { fontSize: fonts.sizes.sm, fontWeight: fonts.weights.semibold, textTransform: 'uppercase', letterSpacing: 1, marginBottom: spacing.sm },
  statsCard: { borderRadius: radius.md, padding: spacing.md, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 4, elevation: 2 },
  statsRow: { flexDirection: 'row', justifyContent: 'space-around' },
  statItem: { alignItems: 'center' },
  statValue: { fontSize: fonts.sizes.xl, fontWeight: fonts.weights.bold },
  statLabel: { fontSize: fonts.sizes.xs, marginTop: 2, textTransform: 'uppercase', letterSpacing: 0.5 },
  calendarCard: { borderRadius: radius.md, padding: spacing.md, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 4, elevation: 2 },
  calendarHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: spacing.md },
  calendarMonth: { fontSize: fonts.sizes.md, fontWeight: fonts.weights.bold },
  calendarRow: { flexDirection: 'row' },
  weekdayLabel: { flex: 1, textAlign: 'center', fontSize: fonts.sizes.xs, fontWeight: fonts.weights.semibold, marginBottom: spacing.xs },
  calendarCell: { flex: 1, aspectRatio: 1, alignItems: 'center', justifyContent: 'center', borderRadius: radius.sm, margin: 2 },
  calendarDay: { fontSize: fonts.sizes.sm, fontWeight: fonts.weights.medium },
  selectedDetail: { marginTop: spacing.md, paddingTop: spacing.md, borderTopWidth: StyleSheet.hairlineWidth, alignItems: 'center' },
  selectedDate: { fontSize: fonts.sizes.md, fontWeight: fonts.weights.semibold },
  selectedStat: { fontSize: fonts.sizes.sm, marginTop: 4 },
